...
internal class MainMenuContextContinueAdventureTest {
    ...
    fun `should start new context when adventure selected`() {
        //given
        val adventureName = "Adventure started long time ago"

        //when
        val actionResult = mainMenuContext.performAction(SelectAdventureToContinue(1, adventureName))

        //then
        assertThat(actionResult).isEqualTo(ChangeContext(PendingAdventureStarted(adventureName)))
    }
